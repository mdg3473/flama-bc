  ...
  <section id="galeria" className="relative py-24 md:py-32 bg-card/40 border-y border-border">
    <div className="container">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 auto-rows-[180px] md:auto-rows-[220px]">
        {items.map((it, i) => (
          <GalleryTile key={i} item={it} index={i} />
        ))}
      </div>
    </div>
  </section>
...
const GalleryTile = ({ item: it, index: i }: { item: (typeof items)[number]; index: number }) => {
  const pop = usePopIn<HTMLDivElement>();
  return (
    <div
      ref={pop.ref}
      style={{ transitionDelay: `${i * 100}ms` }}
      className={`${pop.className} group relative overflow-hidden border-2 border-border hover:border-primary cursor-pointer ${it.span}`}
    >
      <img
        src={it.img}
        alt={it.label}
